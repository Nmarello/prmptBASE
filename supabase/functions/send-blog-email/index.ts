...
function buildEmailHtml(params: {
  ...
  const featureImageHtml = featureImageUrl ? `
    <div style="margin:0 0 24px;">
      <img src="${featureImageUrl}" alt="" style="width:100%; border-radius:6px; display:block;" />
    </div>` : ''
...
Deno.serve(async (req) => {
  ...
  try {
    const { post_title, post_excerpt, post_url, feature_image_url, tag, recipient_emails } = await req.json()

    if (!post_title || !post_url || !recipient_emails?.length) {
      return new Response(JSON.stringify({ error: 'post_title, post_url, and recipient_emails are required' }), { status: 400 })
    }
    ...
    for (const email of recipient_emails) {
      ...
      const html = buildEmailHtml({ postTitle: post_title, postExcerpt: post_excerpt ?? '', postUrl: post_url, featureImageUrl: feature_image_url, tag, unsubscribeUrl })
      ...
      results.push({ email, ok: res.ok, resend: data })
    }
