  ...
  return (
    <div className="relative" ref={panelRef}>
      <button
        ...
        style={{ color: open ? 'var(--pv-text)' : 'var(--pv-text3)' }}
      ...
      >
        ...
        {unread > 0 && (
          <span className={inBottomNav
            ? 'absolute top-0 right-2 min-w-[16px] h-4 flex items-center justify-center rounded-full bg-[#0071e3] text-white text-[10px] font-bold px-1 leading-none'
            : 'absolute -top-0.5 -right-0.5 min-w-[16px] h-4 flex items-center justify-center rounded-full bg-[#0071e3] text-white text-[10px] font-bold px-1 leading-none'
          }>
            {unread > 9 ? '9+' : unread}
          </span>
        )}
        {inBottomNav && <span className="text-[10px] font-medium">Alerts</span>}
      </button>

      {open && (
        <div
          className={inBottomNav
            ? 'fixed bottom-[76px] left-2 right-2 rounded-2xl shadow-xl z-50 overflow-hidden animate-fade-in'
            : 'absolute top-0 left-full ml-2 w-80 max-w-[calc(100vw-16px)] rounded-2xl shadow-xl z-50 overflow-hidden animate-fade-in'
          }
