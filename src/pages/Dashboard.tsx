...
export default function Dashboard() {
  ...
  // Per-model prompt character limits — sourced from provider docs / API constraints
  const PROMPT_LIMITS: Record<string, number> = {
    // Video models — stricter limits
    'luma':               2000,
    'luma-txt2vid':       2000,
    'kling':              2500,
    'kling-txt2vid':      2500,
    'kling-v3':           2500,
    'minimax-txt2vid':    2000,
    'sora2':              4000,
    'sora2-txt2vid':      4000,
    'pika':               2000,
    'ltx-video':          2000,
    ...
    'flux-dev-img2img':   2000,
    'recraft-v3':         1000,
    'recraft-v4-pro':     1000,
    'nano-banana':        2000,
    'nano-banana-edit':   2000,
  ...
  return (
    <div className="flex overflow-hidden" style={{ height: '100dvh', background: 'var(--pv-bg)', color: 'var(--pv-text)', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <aside className="hidden sm:flex flex-col items-center py-4 gap-1 flex-shrink-0 relative z-20" style={{ width: 'calc(60px + env(safe-area-inset-left))', paddingLeft: 'env(safe-area-inset-left)', background: 'var(--pv-surface)', borderRight: '1px solid var(--pv-border)' }}>
        ...
        <SbBtn tip={theme === 'dark' ? 'Blue theme' : theme === 'blue' ? 'Light mode' : 'Dark mode'} onClick={() => setTheme(theme === 'dark' ? 'blue' : theme === 'blue' ? 'light' : 'dark')}>
          {theme === 'light' ? (
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#3A7BF7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className={`flex-1 flex flex-col p-4 sm:p-8 sm:min-h-0 ${result ? 'min-h-[70vw]' : 'min-h-[45vw]'}`}>
              ...
              {result && (
                <div className="flex gap-2 mt-4 flex-wrap">
                  <button onClick={() => downloadFile(result.url, undefined, result.isVideo)}
                ...
                {selectedGenType && template && (
                  <>
                    ...
                    <TemplateForm
                      ...
                      promptMaxChars={selectedModel ? PROMPT_LIMITS[selectedModel.slug] : undefined}
                      onTourSubjectTyped={() => {}}
